...
*/
esp_err_t snd_vendor_status_to_clt(){
    esp_ble_mesh_msg_ctx_t ctx = {0};
    uint32_t opcode;
    esp_err_t err;

    ESP_LOGI(TAG, "status mssg");
    ctx.net_idx = 0;
    ctx.app_idx = 0;
    ctx.addr = 0x0001;
   ...
   mssg_data[1]=composition.vid;
     ...
     err = esp_ble_mesh_server_model_send_msg(&vnd_models[0],&ctx, opcode,2, mssg_data);
            if (err) {
...
esp_err_t snd_light_status_to_clt(){
    esp_ble_mesh_msg_ctx_t ctx = {0};
    uint32_t opcode;
    esp_err_t err;
...
//    mssg_data[0]=1;
     ...
     err = esp_ble_mesh_server_model_send_msg(&root_models[1],&ctx, opcode,0, NULL);
            if (err) {
                ESP_LOGE(TAG, "Failed to send message 0x%06x", ESP_BLE_MESH_MODEL_OP_LIGHT_LIGHTNESS_STATUS);
            }
    else{
          ESP_LOGI(TAG, "mssg send to the ");
    }
    return err;
                                                ...
                                                esp_ble_mesh_lighting_server_cb_param_t *param)
{
    ...
    uint16_t lightness=param->value.state_change.ctl_set.lightness;
    ...
    case ESP_BLE_MESH_LIGHTING_SERVER_STATE_CHANGE_EVT:
            ...
            param->ctx.recv_op == ESP_BLE_MESH_MODEL_OP_LIGHT_CTL_SET_UNACK)
        {
            ...
            if(lightness == 0xff){
                // pre
                u_int16_t colorTemp= param->value.state_change.ctl_set.temperature;
        prev_temp=colorTemp%800;
        prev_temp=100-prev_temp;
            protocolUARTSend3(prev_temp,prev_bright);

            }
            else{
                prev_bright=param->value.state_change.ctl_set.lightness;
                protocolUARTSend3(prev_temp,prev_bright);
            }
    ...
    if (event == ESP_BLE_MESH_CFG_SERVER_STATE_CHANGE_EVT)
    {
        ...
        case ESP_BLE_MESH_MODEL_OP_APP_KEY_ADD:
            ...
            ESP_LOG_BUFFER_HEX("AppKey", param->value.state_change.appkey_add.app_key, 16);
                uint16_t pri_addr=esp_ble_mesh_get_primary_element_address();

            esp_err_t err=esp_ble_mesh_node_bind_app_key_to_local_model(pri_addr,CID_ESP,ESP_BLE_MESH_VND_MODEL_ID_SERVER,param->value.state_change.appkey_add.app_idx);
            if(!err){
                 ESP_LOGI(TAG, "successfully binded app key");
            }
             else{
                 ESP_LOGI(TAG, "failed binded app key %d",err);
            }
            break;
